...
from FairMot.lib.utils.image import gaussian_radius, draw_umich_gaussian, draw_msra_gaussian
from FairMot.lib.utils.utils import xyxy2xywh, generate_anchors, xywh2xyxy, encode_delta
...
class LoadVideo:  # for inference
    def __init__(self, path, img_size=(1088, 608),gap=None):
        ...
        self.gap = gap  # 有gap说明不是用来追踪的，而是让检测用的。 所以要隔gap帧进行一次检测。
        if gap != None:
            self.vn = int(self.cap.get(cv2.CAP_PROP_FRAME_COUNT)/gap)
        else:
    ...
    def __next__(self):
        # Read image
        if self.gap != None:
            self.cap.set(cv2.CAP_PROP_POS_FRAMES, self.gap * self.count)
...
class LoadShortCutVideo:  # for short tracking
    def __init__(self,video, video_time, rect, Output_size, img_size=(1088, 608), multiple = 2):
        ...
        self.cap.set(cv2.CAP_PROP_POS_FRAMES, self.current_frame_index - multiple*self.frame_rate) # 将视频向前调整一秒
...
class LoadSoftWareFile:  # for inference
    ...
    def __next__(self):
        ...
        channel, action_time, img_point, video_parameter = read_subdata(self.action_datas[self.count], self.Videoparameters)
        ...
        if Message[0] == False:
            assert Message[0] is not False, 'Failed to load action {:d}'.format(self.count)
        else:
            img0 = Message[1]
