...
def plot_detections(image, tlbrs, save_dir,dir_id, frame_id, scores=None, color=(255, 0, 0), ids=None):
    im = np.copy(image)
    ...
    for i, det in enumerate(tlbrs):
        ...
        if x2-x1 < 60 or y2 - y1 < 130:
            continue
        else:
            sub_img = image[max(0,y1):min(y2,height),max(0,x1):min(width,x2)]
            cv2.imwrite(os.path.join(save_dir,'{:05d}_{:05d}_{:05d}.jpg'.format(dir_id,frame_id,i)),sub_img)
    return im
