...
class SVHN(nn.Module):
	...
	def save(self, path_to_dir, step, maximum=5):
		path_to_models = glob.glob(os.path.join(path_to_dir, SVHN.CHECKPOINT_FILENAME_PATTERN.format('*')))
		if len(path_to_models) == maximum:
			min_step = min([int(path_to_model.split('/')[-1][6:-4]) for path_to_model in path_to_models])
			path_to_min_step_model = os.path.join(path_to_dir, SVHN.CHECKPOINT_FILENAME_PATTERN.format(min_step))
			os.remove(path_to_min_step_model)

		path_to_checkpoint_file = os.path.join(path_to_dir, SVHN.CHECKPOINT_FILENAME_PATTERN.format(step))
		torch.save(self.state_dict(), path_to_checkpoint_file)
		return path_to_checkpoint_file

	def load(self, path_to_checkpoint_file,step=None):
		self.load_state_dict(torch.load(path_to_checkpoint_file))

	def load_max_step(self, path_to_checkpoint_dir):
		path_to_models = glob.glob(os.path.join(path_to_checkpoint_dir, SVHN.CHECKPOINT_FILENAME_PATTERN.format('*')))
		max_step = max([int(path_to_model.split('/')[-1][6:-4]) for path_to_model in path_to_models])
		path_to_checkpoint_file = os.path.join(path_to_checkpoint_dir, SVHN.CHECKPOINT_FILENAME_PATTERN.format(max_step))
		self.load(path_to_checkpoint_file)
		return max_step

def load_in_Svhn_model(Cfg):
	# Cfg = Config.fromfile(cfg_file)
	Net = SVHN()
	Net.cuda()
	if Cfg.test_cfg.checkpoint is not None:
		path_to_checkpoint_file = Cfg.test_cfg.checkpoint
		Net.load(path_to_checkpoint_file)
		assert os.path.isfile(path_to_checkpoint_file), '%s not found' % path_to_checkpoint_file
		print('SvhnNet restored from file: %s' % path_to_checkpoint_file)
		Net.eval()
	return Net