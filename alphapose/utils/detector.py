...
class DetectedImgsLoader():
    def __init__(self, input_source, cfg, opt, mode='image', batchSize=1, queueSize=128):
        self.cfg = cfg
        self.opt = opt
        self.mode = mode
        self.device = opt.device
        ...
        self.imglist = [img_name for img_name in os.listdir(self.img_dir) if img_name.split('.')[-1] == 'jpg']
    ...
    def start(self):
        # start a thread to detect human in images
        img_preprocess_for_PoseEstimate_worker = self.start_worker(self.img_preprocess_for_PoseEstimate)
    ...
    def img_preprocess_for_PoseEstimate(self):
        for i in range(self.datalen):
            with torch.no_grad():
                orig_img  = cv2.imread(os.path.join(self.img_dir,self.imglist[i]))
                ...
                self.wait_and_put(self.pose_queue, (inp, orig_img, cropped_box, self.imglist[i]))
