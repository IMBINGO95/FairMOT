            ...
            if boxes is None:
                ...
                if self.opt.save_img or self.save_video or self.opt.vis:
                    ...
                    img = vis_frame(orig_img, result,
                                    add_bbox=(self.opt.pose_track | self.opt.tracking | self.opt.showbox))
...
def heat_to_map(orig_img,hm_data,cropped_boxes):

    from alphapose.utils.transforms import heatmap_to_coord_simple
    from alphapose.utils.vis import vis_frame_simple as vis_frame

    pred = hm_data.cpu().data.numpy()
    assert pred.ndim == 4

    pose_coord, pose_score = heatmap_to_coord_simple(pred[0], cropped_boxes)

    keypoints = []
    for n in range(pose_score.shape[0]):
        keypoints.append(float(pose_coord[n,0]))
        keypoints.append(float(pose_coord[n,1]))
        keypoints.append(float(pose_score[n]))

    img = vis_frame(orig_img, pose_coord, pose_score)
